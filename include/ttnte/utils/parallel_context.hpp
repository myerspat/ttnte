...
public:
  ...
  template<typename T>
  MPI_Request inline isend(const T* data, int size, int dest, MPITag tag) const
  {
    MPI_Request request;
    MPI_Isend(data, size, MPITypeMap<T>::value(), dest, static_cast<int>(tag),
