...
def test_elementwise_operations(device, dtype):
    ...
    tt_a = TTState.from_dense(dense_a, eps=1e-12)
    tt_b = TTState.from_dense(dense_b, eps=1e-12)
    ...
    torch.testing.assert_close(
        tt_hadamard.to_dense(), dense_expected_hadamard, rtol=1e-3, atol=1e-4
    )
    ...
    torch.testing.assert_close(
        tt_div.to_dense(), dense_expected_div, rtol=1e-3, atol=1e-4
    )
...
def test_matrix_vector_operations(device, dtype):
    ...
    torch.testing.assert_close(y_fast.to_dense(), y_expected, rtol=1e-3, atol=1e-4)

    # 2. DMRG MV
    y_dmrg = dmrg_mv(tt_A, tt_x, eps=1e-8)
    torch.testing.assert_close(y_dmrg.to_dense(), y_expected, rtol=1e-3, atol=1e-4)

    # 3. AMEn MV
    y_amen = amen_mv(tt_A, tt_x, eps=1e-8)
    torch.testing.assert_close(y_amen.to_dense(), y_expected, rtol=1e-3, atol=1e-4)
...
def test_matrix_matrix_operations(device, dtype):
    ...
    torch.testing.assert_close(
        C_fast.to_dense(interleave=False), C_expected, rtol=1e-3, atol=1e-4
    )

    # 2. AMEn MM
    C_amen = amen_mm(tt_A, tt_B, eps=1e-8)
    torch.testing.assert_close(
        C_amen.to_dense(interleave=False), C_expected, rtol=1e-3, atol=1e-4
    )
...
def test_amen_solve(device, dtype):
    ...
    torch.testing.assert_close(tt_x.to_dense(), x_expected, rtol=1e-2, atol=1e-3)
