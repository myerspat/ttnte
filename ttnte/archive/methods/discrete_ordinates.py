...
class DiscreteOrdinates:
    ...
    def _construct_tensor_trains(self):
        ...
        for i in range(num_octants):
            ...
            def Y(a, m, ordinates, even=True):
                y = (
                    (-1) ** m
                    * np.sqrt(
                        (2 * a + 1)
                        * math.factorial(a - abs(m))
                        / math.factorial(a + abs(m))
                    )
            ...
            for a in range(self._xs_server.num_moments):
                ...
                for k in range(octants.shape[0]):
                    ...
                    S_Intg[
                        ...
                        Y(a, 0, out_ords),
                        self._octant_ords[:, 0] * Y(a, 0, in_ords),
                    )
                    ...
                    for m in range(1, a + 1):
                        for k in range(octants.shape[0]):
                            ...
                            S_Intg[
                                ...
                                Y(a, m, out_ords, even=True),
                                self._octant_ords[:, 0] * Y(a, m, in_ords, even=True),
                            ) + (
                                2
                                * np.outer(
                                    Y(a, m, out_ords, even=False),
                                    self._octant_ords[:, 0]
                                    * Y(a, m, in_ords, even=False),
                                )
                                if num_dim > 2
                                else 0
                            )
                ...
                def scatter_gtg(mat=None):
                    if mat:
                        return self._xs_server.scatter_gtg(mat)[a,]
                    else:
                        return self._xs_server.scatter_gtg()[a,]
