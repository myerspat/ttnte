...
from ttnte.archive import TensorTrain
from ttnte.archive.geometry import Geometry
from ttnte.archive.xs import Server
...
class DiscreteOrdinates(object):
        ...
        for a in range(self._xs_server.num_moments):
            ...
            for k in range(self._octants.shape[0]):
                ...
                S_Intg[
                    ...
                    self._Y(a, 0, out_ords),
                    self._octant_ords[:, 0] * self._Y(a, 0, in_ords),
                )
                ...
                for m in range(1, a + 1):
                    for k in range(self._octants.shape[0]):
                        ...
                        S_Intg[
                            ...
                            self._Y(a, m, out_ords, even=True),
                            self._octant_ords[:, 0] * self._Y(a, m, in_ords, even=True),
                        ) + (
                            2
                            * np.outer(
                                self._Y(a, m, out_ords, even=False),
                                self._octant_ords[:, 0]
                                * self._Y(a, m, in_ords, even=False),
                            )
                            if self._num_dim > 2
                            else 0
                        )
    ...
    @staticmethod
    def _Y(a, m, ordinates, even=True):
        y = (
            (-1) ** m
            * np.sqrt(
                (2 * a + 1) * math.factorial(a - abs(m)) / math.factorial(a + abs(m))
            )
            * lpmv(m, a, ordinates[:, 1])
        )
